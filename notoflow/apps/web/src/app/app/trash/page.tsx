        ...
        <div className="space-y-3">
          ...
          <div className="divide-y divide-border/30 border rounded-xl overflow-hidden bg-card/45 backdrop-blur-sm">
            {archivedPages?.map((page) => (
              <div key={page.id} className="flex items-center justify-between p-4 hover:bg-accent/20 transition-colors">
                <div className="flex items-center gap-3">
                  <span className="text-base shrink-0">{page.icon || "📄"}</span>
                  <div>
                    <h4 className="text-xs font-semibold text-foreground">{page.title || "Sans titre"}</h4>
                    <p className="text-[9px] text-muted-foreground">
                      Archivé le {new Date(page.updatedAt).toLocaleDateString("fr-FR")}
                    </p>
                  </div>
                </div>
